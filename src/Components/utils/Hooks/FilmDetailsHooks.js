...
export const ActorsHook = props => {
  ...
  useEffect(() => {
    ...
    (async function() {
      setIsError(false);
      try {
        getFilmActorsFromApi(props.idFilm).then(actorsApi => {
          tenActors = actorsApi.cast.filter((element, index) => index <= 9);
          setActors(tenActors);
        });
      } catch (error) {
        setIsError(true);
  ...
  _templateActors = actor => {
    ...
    return (
      <View style={styles.actor_container}>
        <Image style={styles.actor_image} source={url_img} key={keyUrl} />
...
export const ImagesHook = props => {
  ...
  useEffect(() => {
    ...
    (async function() {
      setIsError(false);
      try {
        getFilmImagesFromApi(props.idFilm).then(imagesApi => {
          tenImages = imagesApi.backdrops.filter(
  ...
  _templateImages = image => {
    ...
    return (
      <View key={keyUrl} style={styles.image_container}>
        <Image style={styles.image_image} source={url_img} key={keyUrl} />
      </View>
    );
  };
...
export const RecommandationsHook = props => {
  ...
  useEffect(() => {
    ...
    (async function() {
      setIsError(false);
      try {
        getFilmRecommandationsFromApi(props.idFilm).then(filmsRecommandsApi => {
          tenRecommands = filmsRecommandsApi.results.filter(
            (element, index) => index <= 9,
          );
  ...
  _templateRecommandations = recommand => {
    ...
    if (recommand.poster_path != null) {
      url_img = {
        uri: 'https://image.tmdb.org/t/p/w500' + recommand.poster_path,
      };
    } else {
      url_img = require('../../../assets/Images/no-image.jpg');
    }
    return (
      ...
      >
        <Image style={styles.recommand_image} source={url_img} key={keyUrl} />
      </TouchableOpacity>
    );
  };

  return _listRecommandations();
};
